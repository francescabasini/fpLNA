...
class fokker_planck_vectorfield:
    def __init__(self, *, temperature, drag, extent, resolution,
            vectorfield=None, force=None, boundary=fplanck_fmod.boundary.reflecting):
        """
        ...
        self.beta = np.ones_like(self.temperature)
        ...
        self.vectorfield_values = np.zeros_like(self.grid[0]) 
        ...
        self.mobility = np.ones_like(self.drag) # my idea
        
        for i in range(self.ndim):
            #self.diffusion[i] = constants.k*self.temperature[i]/self.drag[i]
            self.diffusion[i] = 1/self.drag[i]
            

        if self.vectorfield is not None:
            #U = self.potential(*self.grid)
            #self.potential_values += U
            V = self.vectorfield(*self.grid)*self.resolution
            #self.vectorfield_values += np.zeros_like(self.grid[0]) 
        
            # grad values need to have same self.resolution
            #self.force_values -= np.gradient(U, *self.resolution)
            # they should be exaclty the same grid as the potential
            #self.force_values += V #va bene
#             #-------------------------------------
            for i in range(self.ndim):
                #dU = np.roll(U, -1, axis=i) - U
                # gradient in each of the axis
                #dU is already V
                self.Rt[i] += self.diffusion[i]/self.resolution[i]**2*np.exp(-self.beta[i]*V/2)

                # dU is the reverse of V so: -V
                #dU = np.roll(U, 1, axis=i) - U
                self.Lt[i] += self.diffusion[i]/self.resolution[i]**2*np.exp(-self.beta[i]*(-V)/2)
#-------------------------------------------------------------------------------------------------------

        # should be all none for me        
...
#                 self.Lt[i] += self.diffusion[i]/self.resolution[i]**2*np.exp(-self.beta[i]*dU/2)
        ...
        # only diffusion
        if self.force is None and self.vectorfield is None:
            for i in range(self.ndim):
    ...
    def what_is_it(self):
        print(self.vectorfield(*self.grid).shape)
